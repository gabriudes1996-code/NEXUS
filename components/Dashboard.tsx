...
const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  const stats = [
    { label: 'Editais Monitorados', value: '142', trend: '+12 hoje', icon: 'M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z', color: 'text-[#C5A059]' },
    { label: 'Participações Ativas', value: '18', trend: '3 em fase de lances', icon: 'M13 10V3L4 14h7v7l9-11h-7z', color: 'text-[#C5A059]' },
    { label: 'Vitórias (Maio)', value: '06', trend: 'R$ 840k total', icon: 'M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z', color: 'text-[#C5A059]' },
    { label: 'Documentação', value: '98%', trend: 'Atualizada', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', color: 'text-[#C5A059]' },
  ];
    ...
    <div className="flex h-screen bg-[#F8FAFC] text-slate-900 overflow-hidden font-sans">
      ...
      <aside className={`bg-[#0F172A] text-white transition-all duration-300 flex flex-col ${isSidebarOpen ? 'w-64' : 'w-20'} z-50`}>
        <div className="p-6 flex items-center gap-3">
          <div className="w-9 h-9 bg-[#0A2342] border border-[#C5A059]/30 rounded-lg flex items-center justify-center flex-shrink-0 shadow-lg shadow-slate-900/40">
            <svg className="w-5 h-5 text-[#C5A059]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          {isSidebarOpen && <span className="font-black text-lg tracking-tight">Nexus <span className="text-[#C5A059]">Studio</span></span>}
        </div>
        ...
        <nav className="flex-1 px-3 space-y-1 mt-6">
          {[
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === item.id ? 'bg-[#C5A059] text-[#0A2342] shadow-xl shadow-[#C5A059]/20' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
            >
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header className="h-16 bg-white/70 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-40">
          <div className="flex items-center gap-6">
            ...
            <h2 className="text-sm font-black text-slate-900 uppercase tracking-widest">Painel Estratégico <span className="text-[#C5A059]">Azul Papel</span></h2>
          </div>
          ...
          <div className="flex items-center gap-5">
            ...
            <button className="relative p-2 text-slate-400 hover:text-[#C5A059] transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
              <span className="absolute top-2 right-2 w-1.5 h-1.5 bg-[#C5A059] rounded-full border-2 border-white"></span>
            </button>
            <div className="w-8 h-8 rounded-full bg-[#C5A059]/10 flex items-center justify-center text-[#C5A059] text-xs font-black border border-white shadow-sm overflow-hidden">
              <img src="https://ui-avatars.com/api/?name=Azul+Papel&background=F5F5F5&color=C5A059&bold=true" alt="Avatar" />
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-8 lg:p-12">
          <div className="max-w-[1200px] mx-auto space-y-10 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                <p className="text-[10px] font-black text-[#C5A059] uppercase tracking-[0.2em] mb-2">Visão Geral de Performance</p>
                <h3 className="text-4xl font-black text-slate-900 tracking-tighter">Resumo de <span className="text-[#C5A059]">Junho</span></h3>
              </div>
              <div className="flex gap-3">
                ...
                <button className="px-5 py-2.5 bg-[#C5A059] text-[#0A2342] rounded-xl text-xs font-bold hover:bg-[#E5C789] transition-all shadow-xl shadow-[#C5A059]/10 flex items-center gap-2">
              ...
              {stats.map((stat, i) => (
                <div key={i} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                  <div className="flex justify-between items-start mb-6">
                    <div className={`w-10 h-10 bg-slate-50 group-hover:bg-[#C5A059]/10 transition-colors flex items-center justify-center rounded-xl ${stat.color}`}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
                  ...
                  <button className="text-[#C5A059] text-[10px] font-black uppercase hover:underline">Ver Histórico</button>
                </div>
                      ...
                      {licitacoes.map((row, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                          <td className="px-8 py-5">
                            <p className="text-[11px] font-black text-slate-900 uppercase leading-none mb-1 group-hover:text-[#C5A059] transition-colors">{row.orgao}</p>
                          ...
                          <td className="px-8 py-5 text-center">
                            <span className={`px-2.5 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest ${row.status === 'Habilitado' ? 'bg-[#C5A059]/10 text-[#C5A059]' :
              ...
              <div className="space-y-6">
                <div className="bg-[#111827] rounded-[2rem] p-8 text-white relative overflow-hidden group border border-slate-800">
                  <div className="relative z-10">
                    <p className="text-[10px] font-black text-[#C5A059] uppercase tracking-widest mb-3">Próximo Pregão</p>
                ...
                <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm">
                  ...
                  <button className="w-full mt-6 py-4 border border-slate-100 hover:border-[#C5A059]/20 hover:bg-[#C5A059]/5 text-[#C5A059] rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">Atendimento Direto</button>
                </div>
              </div>
            </div>
